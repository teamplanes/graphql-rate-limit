...
import { createRateLimitDirective, getFieldIdentity, validateResolve } from './field-directive';
...
test('validateResolve should return true for full stores', async (t) => {
	...
	store.setForIdentity({ contextIdentity: '1', fieldIdentity: 'myField' }, [ Date.now(), Date.now() ]);
	t.true(await validateResolve(store, { contextIdentity: '1', fieldIdentity: 'myField' }, { max: 1, window: 1000 }));
	t.false(await validateResolve(store, { contextIdentity: '1', fieldIdentity: 'myField' }, { max: 4, window: 1000 }));
});
