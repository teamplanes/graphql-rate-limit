...
import { Options, Identity } from './types';
 ...
 * @param resolveArgs 
 */
const getFieldIdentity = (fieldName: string, identityArgs: ReadonlyArray<string>, resolveArgs: any) => {
 ...
 * whether the the resolve can be accessed. validateResolve will return true if it can be accessed
 * and false if not.
 * 
 * @param store 
 * @param identity 
 * @param options 
 */
const validateResolve = async (store: Store, identity: Identity, options: Options) => {
 ...
 * @param customConfig 
 */
const createRateLimitDirective = (customConfig: GraphQLRateLimitConfig = {}): any => {
	...
	class GraphQLRateLimit extends SchemaDirectiveVisitor {
		...
		public visitFieldDefinition(field: GraphQLField<any, any>): void {
			...
			field.resolve = async (...args) => {
				...
				const message = this.args.message || `You are trying to access '${name}' too often'`;
				const identityArgs = this.args.identityArgs || DEFAULT_FIELD_IDENTITY_ARGS;
